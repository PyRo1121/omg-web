...
function calculateAchievementProgressScore(total: number, unlocked: number): number {
  ...
  if (total === 0) {
    return 15;
  } // Default if no achievements defined
...
function calculateResolutionSatisfactionScore(tickets: SupportTicket[]): number {
  ...
  if (resolvedWithRating.length === 0) {
    return 30;
  } // Default if no rated tickets
...
export function calculateChurnRisk(score: HealthScoreResult, trends: HealthTrends): ChurnRisk {
  ...
  if (trends.engagementTrend === 'declining') {
    riskPoints += 20;
  } else if (trends.engagementTrend === 'stable') {
    riskPoints += 5;
  }

  // Inactivity risk (0-20 points)
  if (trends.daysSinceLastActivity > 14) {
    riskPoints += 20;
  } else if (trends.daysSinceLastActivity > 7) {
    riskPoints += 10;
  } else if (trends.daysSinceLastActivity > 3) {
    riskPoints += 5;
  }
...
export function predictChurnProbability(history: ChurnHistoryRecord[]): ChurnPrediction {
  ...
  if (current.tier === 'free') {
    logit += weights.tierFree;
  } else if (current.tier === 'team') {
    logit += weights.tierTeam;
  } else {
    logit += weights.tierEnterprise;
  }
