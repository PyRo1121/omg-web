...
export const CustomerHealthHub: Component<CustomerHealthHubProps> = props => {
  ...
  const riskLevel = createMemo(() => {
    ...
    if (churn >= 0.7) {
      return { level: 'critical', color: 'text-flare-500', bg: 'bg-flare-500/10' };
    }
    if (churn >= 0.4) {
      return { level: 'high', color: 'text-flare-400', bg: 'bg-flare-500/10' };
    }
    if (churn >= 0.2) {
      return { level: 'medium', color: 'text-solar-400', bg: 'bg-solar-500/10' };
    }
