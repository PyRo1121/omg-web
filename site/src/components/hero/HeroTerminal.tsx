...
const HeroTerminal: Component = () => {
  ...
  return (
    <div class="perspective-1000 mx-auto w-full max-w-2xl">
      <GlassCard
        class="rotate-y--6 rotate-x-6 transform transition-transform duration-700 ease-out hover:rotate-x-0 hover:rotate-y-0"
      ...
      >
        ...
        <div class="flex items-center gap-2 border-b border-white/10 bg-white/5 px-4 py-3">
          ...
          <div class="ml-2 font-mono text-xs text-white/40 select-none">omg — bash</div>
        </div>

        {/* Terminal Body */}
        <div class="min-h-[320px] bg-black/20 p-6 font-mono text-sm sm:text-base">
          <div class="flex flex-col gap-1">
            <For each={lines()}>
              {line => (
                <div class={line.startsWith('$') ? 'text-cyan-400' : 'text-white/70'}>{line}</div>
              )}
            </For>
            <div class="text-cyan-400">
              ...
              {currentLine()}
              <span
                class={`ml-1 inline-block h-5 w-2 bg-cyan-400 align-middle ${cursorVisible() ? 'opacity-100' : 'opacity-0'}`}
              />
            </div>
          </div>
        </div>
