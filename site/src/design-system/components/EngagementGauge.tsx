  ...
  return (
    ...
    >
      ...
      <div class="absolute inset-0 flex flex-col items-center justify-center">
        <div class={cn(metric().bgClass, 'mb-1 rounded-xl p-2')}>
          <Icon size={config().iconSize} style={{ color: metric().color }} />
        </div>
        <span class={cn('font-display font-black text-white tabular-nums', config().fontSize)}>
          {local.value >= 1000
            ? `${(local.value / 1000).toFixed(1)}k`
            : local.value.toLocaleString()}
        </span>
        <span class={cn('text-nebula-500 font-bold tracking-widest uppercase', config().labelSize)}>
          {local.label}
        </span>
  ...
  return (
    <div class={cn('space-y-6', local.class)} {...others}>
      ...
      <Show when={local.showStickiness}>
        <div class="bg-void-850 flex items-center justify-center gap-6 rounded-2xl border border-white/5 p-6">
          <div class="flex-1">
            ...
            <div class="mt-2 flex items-baseline gap-2">
              <span
                ...
                style={{ color: metricConfig.stickiness.color }}
              >
          ...
          <div class="flex flex-col gap-2 text-sm">
            <div class="flex items-center gap-2">
              <div class="bg-nebula-600 h-2 w-2 rounded-full" />
              <span class="text-nebula-500">Dormant</span>
              <span class="text-nebula-400 ml-auto">&lt;10%</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="bg-plasma-500 h-2 w-2 rounded-full" />
              <span class="text-nebula-500">Casual</span>
              <span class="text-nebula-400 ml-auto">10-25%</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="bg-electric-500 h-2 w-2 rounded-full" />
              <span class="text-nebula-500">Regular</span>
              <span class="text-nebula-400 ml-auto">25-50%</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="bg-aurora-500 h-2 w-2 rounded-full" />
              <span class="text-nebula-500">Active</span>
              <span class="text-nebula-400 ml-auto">50-75%</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="bg-solar-400 h-2 w-2 rounded-full" />
              <span class="text-nebula-500">Power</span>
              <span class="text-nebula-400 ml-auto">75%+</span>
            </div>
          </div>
        </div>
      </Show>
    ...
    <div class="space-y-2">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <span class="text-nebula-300 text-sm font-medium">{props.label}</span>
        </div>
        <div class="flex items-center gap-2">
      ...
      <div class="bg-void-700 h-2 overflow-hidden rounded-full">
        <div
          class={cn(
            'ease-smooth h-full rounded-full transition-all duration-1000',
...
const EngagementCard: Component<{
  ...
  return (
    <div
      class={cn(
        'bg-void-850 rounded-2xl border border-white/5 p-5',
        'transition-all duration-300 hover:translate-y-[-2px] hover:border-white/10',
        'group'
      )}
    >
      ...
      <div class="mt-4">
        ...
        <div class="font-display mt-1 text-3xl font-black text-white tabular-nums">
          {props.value >= 1000000
...
const StickinessBar: Component<{ value: number; animated?: boolean }> = props => {
  ...
  return (
    <div class="bg-void-700 relative h-3 overflow-hidden rounded-full">
      <div class="absolute inset-0 flex">
        <div class="border-void-600 w-[10%] border-r" />
        <div class="border-void-600 w-[15%] border-r" />
        <div class="border-void-600 w-[25%] border-r" />
        <div class="border-void-600 w-[25%] border-r" />
        <div class="w-[25%]" />
      </div>
      <div
