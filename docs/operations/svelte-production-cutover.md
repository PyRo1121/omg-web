...
- [x] With dedicated production OAuth inputs present, the 2026-08-31 production Alchemy deployment created only the unattached `omgsveltesite-website-prod-dlaqgfttmir2ky5x` Worker, stage-scoped bindings, limiters, and generated auth secret. It has no workers.dev URL, hostname, Worker Route, D1 resource action, or existing-Worker adoption. The latest source-only update produced deployment `cea2e10e-37db-497a-b361-3a54e4cb07b5`, serving Worker version `d2d77460-f528-4a10-a2c2-2cacdb223bf4` at 100%; the follow-up production plan is `2 to noop`.
