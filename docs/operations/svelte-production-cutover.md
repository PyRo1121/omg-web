...
- [x] The immediate `omg-saas` code rollback target is `ae49849a-2309-434f-9684-6cd6636c9b8c`; the exact command is `cd workers/api && npx wrangler rollback ae49849a-2309-434f-9684-6cd6636c9b8c --message "rollback bounded provider responses"`. The version is present, but rollback was deliberately not executed because the new version passed its smoke gates.
...
- [x] With dedicated production OAuth inputs present, the 2026-08-31 production Alchemy deployment created only the unattached `omgsveltesite-website-prod-dlaqgfttmir2ky5x` Worker, stage-scoped bindings, limiters, and generated auth secret. It has no workers.dev URL, hostname, Worker Route, D1 resource action, or existing-Worker adoption. The latest source-only update produced deployment `37f826a8-ae20-4baf-bf38-7a09508dae83`, serving Worker version `c32e147b-775e-49f2-ad9d-ce3ed2bc6ee4` at 100%; the follow-up production plan is `2 to noop`.
