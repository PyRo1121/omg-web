...
- [ ] Checkout remains blocked: authenticated Pro Checkout reaches `omg-saas` and the page fails closed with `Checkout is temporarily unavailable.` Checkout Sessions Write is present, but the replacement restricted key belongs to a different live Stripe account from the configured products and prices. Rotate only to an equivalently restricted key from the catalog-owning account, repeat Checkout, expire the unpaid characterization session, and record the successful redirect before approving the hostname window.
...
- [ ] User-controlled authenticated Checkout characterization is blocked on live Stripe account alignment; the configured catalog and the dedicated restricted key must belong to the same account, and the application continues to fail closed with a bounded generic message.
