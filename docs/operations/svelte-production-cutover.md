...
- [ ] Production OAuth now fails closed before planning unless stage-specific credentials exist. Live authorization discovery confirmed that production and shadow currently emit the same GitHub OAuth client ID. That shared value was not retained in the production-only keyring slot: provision a dedicated production OAuth app as `PRODUCTION_GITHUB_CLIENT_ID` and `PRODUCTION_GITHUB_CLIENT_SECRET` before the production plan can run. The existing write-only production secret cannot be recovered from Cloudflare.
