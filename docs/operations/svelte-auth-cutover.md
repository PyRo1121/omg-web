...
stage-specific production GitHub inputs exist. A shadow plan is expected to be
a no-op only when its deployed source matches the checkout; after reviewed code
changes it may show a `Website` update, which must be reviewed and deployed
separately before requiring a follow-up no-op plan. After rotation, deploy and
...
| #   | Action                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Rollback                                                                                                                                                                                                                   |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Completed 2026-08-31 — bind production GitHub credentials at deploy time.** A dedicated production GitHub OAuth app uses callback `https://omg.latham.cloud/api/auth/callback/github`; its values are stored as `PRODUCTION_GITHUB_CLIENT_ID` and `PRODUCTION_GITHUB_CLIENT_SECRET` in the `omg-web-alchemy` keyring. The wrapper maps only those stage-specific values for `--stage prod` and refuses fallback to the shadow app. Distinct client IDs and a credential-free production plan were verified. The current shadow plan contains only its expected pending code update and stable auth-secret no-op; deploy and characterize that update separately. | Remove or correct the production-specific inputs; the shadow stage remains untouched. Leave the unused production app disabled or remove it only after rollback is complete.                                               |
