...
| `audit-y2.md`  | **Open**        | Several Svelte server schema/type duplicates remain. They should be reduced only where one domain contract can replace both declarations without creating generic wrappers or coupling route forms to private Worker payloads.                                                                                                                            |
| `audit-y3.md`  | **Open**        | Shared security policy and several boundary helpers are centralized, but bounded readers, email vocabulary, and small decode helpers still need a call-site-by-call-site deletion test. Broad D1/error/UI abstractions proposed by the report are not automatically approved.                                                                             |
...
3. Adjudicate and reduce the concrete Svelte server duplicates in `audit-y2.md` and `audit-y3.md`.
4. Reduce `piolium` evidence fan-out while preserving one canonical historical proof per finding.
5. Complete `audit-y6.md` with the Rust remediation stream.
