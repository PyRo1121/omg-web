...
- Checkout alignment is verified. The installed restricted key was rotated into the catalog-owning OMG account, authenticated Checkout reached `omg-saas` with status `200`, headed Helium reached `https://checkout.stripe.com` without payment, and the one unpaid characterization session was expired. Live Stripe reports zero open sessions with complete pagination.
...
| D1 rows read/written | 5M read / 100k written per day       | Single indexed database. Daily pruning bounds docs, site and CLI telemetry, 12-month usage and offer leads, 30-day audit logs, and processed Stripe events for 90 days. Licensing and organization rows remain durable. |
...
The expanded CLI analytics and usage-retention queries are committed but still require the next reviewed `omg-saas` deployment. A read-only production query on 2026-09-01 found zero rows in every affected analytics and usage table, so the deployment delay is not retaining customer telemetry.
...
4. Inbound delivery for `support@latham.cloud` is verified. On 2026-09-01 Cloudflare rejected Email Routing enablement before mutation because the domain already uses Apple MX records. Replacing those records would interrupt existing domain mail, so Cloudflare Email Routing remains disabled by design. The address instead uses the existing Apple custom-email-domain service. An approved non-sensitive retry from the independently hosted `codeloud.xyz` sender reached the Apple inbox. The verified retry was classified as junk, so operators must monitor junk filtering while treating `support@latham.cloud` as the canonical privacy-request channel.
5. Verify Workers observability after first real traffic and exercise the rollback path once: enumerate versions with `npx wrangler deployments list`, then revert with `npx wrangler rollback` (<https://developers.cloudflare.com/workers/wrangler/commands/#rollback>, <https://developers.cloudflare.com/workers/versioning/>).
