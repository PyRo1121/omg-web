...
4. Enable and test inbound Email Routing for `support@latham.cloud` before deploying privacy policy 2.1 or cutting over to Svelte. A read-only check on 2026-09-01 found routing disabled, no explicit or catch-all forwarding rule, and exactly one verified destination address. Enabling routing changes zone DNS and creating the forwarding rule changes customer-message delivery; both require a reviewed provider-write checkpoint and temporary least-privilege Email Routing and DNS permissions. Use a non-sensitive approved smoke message, verify receipt, then retain the route as the canonical privacy-request channel.
